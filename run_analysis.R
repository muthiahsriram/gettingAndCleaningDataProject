...
features<-read.csv(paste0(file,"/UCI HAR Dataset/features.txt",collapse=NULL),sep = "",header = F,col.names=c("s.no","Features"),stringsAsFactors=F)
features<-features[,c("Features")]
test_x<-read.csv2(paste0(file,"/UCI HAR Dataset/test/X_test.csv",collapse=NULL), sep = "",header=F,col.names=features,stringsAsFactors=F)
test_x<-test_x[,grep("mean|std",colnames(test_x),ignore.case=T)]
test_label<-read.csv2(paste0(file,"/UCI HAR Dataset/test/y_test.csv",collapse=NULL), sep = "",header=F,col.names=c("label"),stringsAsFactors=F)
test_subject<-read.csv2(paste0(file,"/UCI HAR Dataset/test/y_test.csv",collapse=NULL), sep = "",header=F,col.names=c("subjects"),stringsAsFactors=F)
test<-cbind(test_subject,test_label,test_x)


train_x<-read.csv2(paste0(file,"/UCI HAR Dataset/train/X_Train.txt",collapse=NULL),header=F,sep="",col.names=features,stringsAsFactors=F)
train_x<-train_x[,grep("mean|std",colnames(train_x),ignore.case=T)]
train_label<-read.csv2(paste0(file,"/UCI HAR Dataset/train/y_train.txt",collapse=NULL),header=F,col.names="label",sep="",stringsAsFactors=F)
train_subject<-read.csv2(paste0(file,"./UCI HAR Dataset/train/subject_train.txt",collapse=NULL),header=F,col.names=('subjects'),sep="",stringsAsFactors=F)
train<-cbind(train_subject,train_label,train_x)

test_train<-rbind(test,train)

activity_label<-read.csv2(paste0(file,"/UCI HAR Dataset/activity_labels.txt",collapse=NULL),header=F,sep="",col.names=c("label","activity"))
